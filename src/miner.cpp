...
void static DatacoinCPUMiner(int nThread, int numThreads, const CChainParams& chainparams)
{
    ...
    RenameThread(strprintf("datacoin-miner-%s", nThread).c_str());
    ...
    if (!EnsureWalletIsAvailable(pwallet, false) || pwallet == NULL)
        throw std::runtime_error(strprintf("Datacoin miner %s wallet unavailable (mining requires a wallet)", nThread));
    ...
    if (!coinbase_script) {
        LogPrintf("Keypool ran out for miner %s. Refilling...\n", nThread);
		{
			...
			if (pwallet->GetKeyPoolSize() < 1) {
                LogPrintf("Error refreshing keypool. Terminating miner thread %s.\n", nThread);
				return;
			}
		}		
		pwallet->GetScriptForMining(coinbase_script);		
		if (!coinbase_script) {
            LogPrintf("Can't get coinbase script from keypool. Terminating miner thread %s.\n", nThread);
			return;
		}
    }
    //throw an error if no script was provided
    if (coinbase_script->reserveScript.empty()) {
        LogPrintf("No coinbase script available. Terminating miner thread %s.\n", nThread);
		return;
    }
    ...
    if (fDebug && gArgs.GetBoolArg("-printmining", false))
        LogPrintf("DatacoinMiner %s: Setting initial extra nonce to %u\n", nThread, nExtraNonce);
    ...
    try { while(true) {
        ...
        CBlockIndex* pindexPrev = pindexBestHeader; // obtained from validation.h rather than chainActive.Tip()
        ...
        if (fDebug && gArgs.GetBoolArg("-printmining", false))
            LogPrintf("Running DatacoinMiner %s with %u transactions in block (%u bytes)\n", nThread, static_cast<unsigned int>(pblock->vtx.size()),
        ...
        while(true)
        {
            ...
            if (nMillisNow - nHPSTimerStart > 6000)
            {
                LOCK(cs);
                nMillisNow = GetTimeMillis();
                if (nMillisNow - nHPSTimerStart > 6000)
                {
                    ...
                    if (nMillisNow - nLogTime > 5900)
                    {
                        nLogTime = nMillisNow;
                        if (fLogTimestamps)
                            LogPrintf("DatacoinMiner %s - primemeter %9.0f primes/hr %9.0f tests/hr %3.8f chains/dy %3.8f blocks/dy.\n", nThread,
                                      dPrimesPerMinute * 60.0, dTestsPerMinute * 60.0, dChainsPerDay, dBlocksPerDay);
                        else
                            LogPrintf("DatacoinMiner %s - %s primemeter %9.0f primes/hr %9.0f tests/hr %3.8f chains/dy %3.8f blocks/dy.\n", nThread,
                                      DateTimeStrFormat("%Y-%m-%d %H:%M:%S", nLogTime / 1000).c_str(),
                                      dPrimesPerMinute * 60.0, dTestsPerMinute * 60.0, dChainsPerDay, dBlocksPerDay);
                        if (fDebug && gArgs.GetBoolArg("-printmining", false))
            ...
            if (fNewBlock)
            {
                ...
                if (nRoundNum >= nRoundSamples)
                {
                    ...
                    if (fDebug && gArgs.GetBoolArg("-printprimorial", false))
                        LogPrintf("DatacoinMiner %s : Rounds total: num=%u primorial=%u block/s=%3.12f\n", nThread, nRoundNum, nPrimorialMultiplier, dAverageBlockExpected);
                ...
                if (fDebug && gArgs.GetBoolArg("-printmining", false))
                {
                    ...
                    LogPrintf("DatacoinMiner %s: Round primorial=%u tests=%u primes=%u time=%uus pprob=%1.6f pprob2=%1.6f pprobextra=%1.6f tochain=%6.3fd expect=%3.12f expectblock=%3.12f\n", nThread, nPrimorialMultiplier, nRoundTests, nRoundPrimesHit, (unsigned int) nRoundTime, dPrimeProbabilityBegin, dPrimeProbabilityEnd, dExtraPrimeProbability, ((dTimeExpected/1000000.0))/86400.0, dRoundChainExpected, dRoundBlockExpected);
                }
                ...
                if (nFixedPrimorial == 0 && nAdjustPrimorial != 0) {
                    if (nAdjustPrimorial > 0)
                    {
                        if (!PrimeTableGetNextPrime(nPrimorialMultiplier))
                            error("DatacoinMiner %s : primorial increment overflow.", nThread);
                    }
                    else if (nPrimorialMultiplier > nPrimorialHashFactor)
                    {
                        if (!PrimeTableGetPreviousPrime(nPrimorialMultiplier))
                            error("DatacoinMiner %s : primorial decrement overflow.", nThread);
                    }
                    Primorial(nPrimorialMultiplier, mpzPrimorial);
                    nAdjustPrimorial = 0;
                }
